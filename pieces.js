...
class Piece {
  ...
  constructor(x, y, color,lock) {
    ...
    this.y=y;
    this.lock=lock;
    this.body.setbounds(new rect(this.x*100,this.y*100,100,100));
...
class Queen extends Piece {
  ...
  this.body.render();
    ...
    moves(grid) {
        let total = [];
        let temp = new Rook(this.x, this.y, this.color);
        total=total.concat(temp.moves(grid));
        temp = new Bishop(this.x, this.y, this.color);
        total=total.concat(temp.moves(grid));
        return total;
    }
  attacks(grid){
